...
export default function NexusUtilizationPage() {
  ...
  const fetchUtilization = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5000/api/nexus/utilization");
      setData(res.data);
    } catch (err) {
  ...
  const takeSnapshot = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/api/nexus/utilization/snapshot");
      ...
      <main className="main-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button 
              onClick={() => window.open('http://localhost:5000/metrics', '_blank')}
                    ...
                    <tr key={tenant.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '16px 24px' }}>
                        <span style={{ 
                          ...
                          background: tenant.plan?.toLowerCase() === 'professional' ? '#f0f9ff' : '#f8fafc',
