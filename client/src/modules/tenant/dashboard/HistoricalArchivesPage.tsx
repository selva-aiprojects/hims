  ...
  const printInvoice = async (bill: any) => {
    ...
    const itemRows = items.map((item: any, idx: number) => `
      <tr>
        ...
        <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right;">₹ ${Number(item.unit_price).toLocaleString()}</td>
        <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right;">₹ ${Number(item.discount_amount || 0).toLocaleString()}</td>
        <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 700;">₹ ${Number(item.amount).toLocaleString()}</td>
      </tr>
    `).join("");

    const hospitalName = localStorage.getItem("tenantName") || "HEALTHEZEE CLINICS";
        ...
        <body>
          ...
          <div class="totals-container">
            <div class="totals-box">
              <div class="total-row">
                <span>Subtotal:</span>
                <span>₹ ${Number(bill.subtotal || 0).toLocaleString()}</span>
              </div>
              <div class="total-row">
                <span>Tax Total:</span>
              ...
              ${bill.insurance_claim_amount > 0 ? `
                <div class="total-row">
                  ...
                  <span style="color: #10b981; font-weight: 600;">- ₹ ${Number(bill.insurance_claim_amount).toLocaleString()}</span>
                </div>
                <div class="total-row">
                  <span>Patient Copay (${Math.round((bill.patient_copay_amount / bill.total) * 100)}%):</span>
                  <span>₹ ${Number(bill.patient_copay_amount).toLocaleString()}</span>
                </div>
              ` : ''}
              <div class="total-row grand-total">
                <span>Total Paid:</span>
                <span>₹ ${Number(bill.insurance_claim_amount > 0 ? bill.patient_copay_amount : bill.total).toLocaleString()}</span>
              </div>
            </div>
          </div>
