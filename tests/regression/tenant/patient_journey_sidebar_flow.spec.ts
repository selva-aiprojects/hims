...
test.describe('Patient journey from sidebar', () => {
  test('sidebar order and OPD/IPD journey smoke', async ({ page }) => {
    ...
    const journey = ['OPD Registration', "Doctor's Queue", 'Consultation Desk', 'Admission Desk', 'IPD Bed Map', 'IPD Census & Daycare', 'Discharge Summaries'];
    const presentJourney = journey.filter((label) => labels.includes(label));
    expect(presentJourney).toEqual(journey);

    await page.getByText('Consultation Desk', { exact: true }).click();
    await expect(page).toHaveURL(/\/tenant\/opd\/consultation/);
    await expect(page.getByText('No Active Patient')).toBeVisible();

    await page.getByText('OPD Registration', { exact: true }).click();
    await expect(page).toHaveURL(/\/tenant\/opd\/registration/);
    await expect(page.getByText('OPD Professional Intake Desk')).toBeVisible();
    ...
    await page.getByText("Doctor's Queue", { exact: true }).click();
    ...
    const diagnosis = page.locator('select').first();
    if (await diagnosis.locator('option').count() > 1) {
    ...
    await page.getByText('IPD Census & Daycare', { exact: true }).click();
    ...
    await page.getByText('Discharge Summaries', { exact: true }).click();
